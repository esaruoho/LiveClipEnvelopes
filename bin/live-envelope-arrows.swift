...
//--------------------------------------------------------------------------------
// Where live-envelope is. Same probe order the app and the macros use, app bundle
// FIRST: that copy is signed with the app, so it runs under the app's Accessibility
// grant. A hardcoded path meant this only ever worked on one machine, in one checkout.
//--------------------------------------------------------------------------------
let SIBLING_CANDIDATES = [
    "/Applications/Live Envelopes.app/Contents/MacOS/live-envelope",
    NSHomeDirectory() + "/Applications/Live Envelopes.app/Contents/MacOS/live-envelope",
    "/usr/local/bin/live-envelope",
    NSHomeDirectory() + "/.local/bin/live-envelope",
    // A dev checkout: built alongside this binary by install.sh --with-arrows.
    URL(fileURLWithPath: CommandLine.arguments[0]).deletingLastPathComponent()
        .appendingPathComponent("live-envelope").path,
]

func liveEnvelopePath() -> String? {
    SIBLING_CANDIDATES.first { FileManager.default.isExecutableFile(atPath: $0) }
}
...
func run(_ argument: String) {
    guard let binary = liveEnvelopePath() else {
        log("live-envelope not found — install Live Envelopes.app in /Applications")
        return
    }
    let process = Process()
    ...
    do { try process.run() } catch { log("could not run \(binary): \(error)") }
}
